const Game = {
  ...
  createEnemies() {
    ...
    if (this.framesCounter % 400 == 50) {
      ...
      const enemy = new Enemy(this.ctx, this.framesCounter, this.player, this.width, this.height, posStartY, 'image 21.jpg')
  ...
  explodeAsteroid() {
    this.asteroids = this.asteroids.filter(asteroid => {
      ...
      return !this.player.shots.some(shot => {
        ...
        if (isImpact) {
          this.score += 1
          this.removeElementFromArray(shot, this.player.shots)      //marcador de ptos en HTML
        }
  ...
  explodeEnemy() {
    this.enemies = this.enemies.filter(enemy => {
      ...
      return !this.player.shots.some(shot => {
        ...
        document.querySelector('.score').innerHTML = this.score;  //marcador de puntuacion
        return isImpact
      })
    })
  },
  ...
  reset() {
    this.background = new Background(this.ctx, this.width, this.height, "./img/escenario3.jpg")
    this.player = new Player(this.ctx, this.width, this.height, this.gravity, 50, 300, 300, 150, "player.png", this.keys)
